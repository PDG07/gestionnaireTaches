...
public record TaskDto (Long id, String title, String description, String status, String priority, String deadline, String category, UserDto user){
    ...
    public static TaskDto taskDto (Task task) {
        UserDto userDto = new UserDto(task.getUser().getId(), task.getUser().getUsername(), task.getUser().getPassword(), task.getUser().getTasks().stream().map(TaskDto::taskDto).collect(Collectors.toSet()));
